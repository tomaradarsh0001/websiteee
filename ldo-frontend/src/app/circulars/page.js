...
const Page = () => {
  ...
  const { lang } = useContext(LangContext);
    const [tableData, setTableData] = useState([{id:'',docName:Translate.noDocFound[lang],uploadDate:'',action:''}])
    const columns = [
      ...
      useEffect(() => {
          const fetchData = async () => {
              try {
                  const response = await fetch(API_HOST + 'officeDocs/4/' + lang)
    ...
    <div className="contact-us">
      <PageHeader pageTitle={lang == "hindi" ? "परिपत्र":"Circulars"} language={lang}/>
    ...
    <div className='w-full px-4'>
        ...
        <h2 className='themeTitle text-2xl lg:text-4xl font-bold text-center'>{lang == "hindi" ? "परिपत्र":"Circulars"}</h2>
        </div>
    </div>
</div>
