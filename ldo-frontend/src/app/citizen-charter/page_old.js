...
const Page = () => {
  const { lang } = useContext(LangContext);
    ...
    useEffect(() => {
      const fetchData = async () => {
          try {
              const response = await fetch(API_HOST + 'componentData/Citizen Charter/' + lang)
  ...
  return componentContent.sections == undefined ? (
    ...
    <div className="about-us">
      <PageHeader pageTitle={lang == "hindi" ? "नागरिक चार्टर":"Citizen Charter"} language={lang}/>
      <div className='introduction-container bg-slate-50 px-4 md:px-4 py-10 lg:py-10 lg:px-8 xl:px-10 2xl:px-24'>
        <div className='w-full md:w-2/4 lg:w-3/4 m-auto px-4'>
          <div className='title-group2 pb-2 lg:pb-5 mb-5' data-aos="fade-up" data-aos-duration="1000">
            <h2 className='themeTitle text-2xl md:text-3xl lg:text-4xl font-bold text-center'>{componentContent.heading}</h2>
          </div>
        </div>
          ...
          <div className='w-full lg:w-full 2xl:w-3/4 px-4'>
            <div className='relative'>
              <div className='mb-5' data-aos="zoom-out-right" data-aos-duration="1000">
                <Image src={CitizenCharterBG} alt='Introduction' className='w-full round-[10px] mt-5' />
              </div>
            </div>
          </div>
          <div className='w-full relative px-4 py-4 md:py-5 md:px-5 xl:w-3/6 lg:transform-none xl:-translate-y-2/4 float-content rounded-2xl xl:absolute xl:top-1/2 xl:right-0'>
            <div data-aos="fade-left" data-aos-duration="1000">
              <div className='title-group-subtitle pb-2 lg:pb-5 mb-5' data-aos="fade-right" data-aos-duration="1000">
                <h2 className='themeTitle text-xl lg:text-3xl font-bold'>{componentContent.sections[0].title}</h2>
              </div>
      ...
      <div className='introduction-container px-4 md:px-4 py-10 lg:py-10 lg:px-8 xl:px-10 2xl:px-24'>
        <div className='w-full md:w-2/4 lg:w-3/4 m-auto px-4'>
          <div className='title-group2 pb-2 lg:pb-5 mb-5' data-aos="fade-up" data-aos-duration="1000">
            <h2 className='themeTitle text-2xl md:text-3xl lg:text-4xl font-bold text-center'>{componentContent.sections[1].title}</h2>
          </div>
        </div>
        <div className='block xl:flex items-center w-full relative'>
          <div className='w-full px-4'>
            <div className='relative'>
              <div className='mb-10' data-aos="fade-up" data-aos-duration="1000" dangerouslySetInnerHTML={{ __html: componentContent.sections[1].content }} />
              {/* </div> */}
