...
const Page = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(API_HOST + "officeDocs/6/" + lang);
    ...
    return tableData.length == 0 ? (
        ...
        <>
            <PageHeader
                pageTitle={lang == "hindi" ? "मीडिया गैलरी" : "Media Gallery"}
                language={lang}
            />
                ...
                <div className="w-full px-4">
                    ...
                    >
                        <h2 className="themeTitle text-2xl lg:text-4xl font-bold text-center">
                            {lang == "hindi" ? "मीडिया गैलरी" : "Media Gallery"}
                        </h2>
                    </div>
                </div>
