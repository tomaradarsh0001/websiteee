...
const Page = () => {
  ...
  const { lang } = useContext(LangContext);
    ...
    const columns = [
      ...
      useEffect(() => {
          const fetchData = async () => {
              try {
                  const response = await fetch(API_HOST + 'officeDocs/7/' + lang)
    ...
    <>
  <PageHeader pageTitle={lang == "hindi" ? "संसाधन संग्रह":"Resource Library"} language={lang} />
  <div className='whatwedo px-4 md:px-6 pt-10 lg:px-8 xl:pt-20 xl:px-10 2xl:px-24 2xl:pt-10'>
    <div className='w-full px-4'>
        <div className='title-group2 pb-2 lg:pb-5 mb-5' data-aos="fade-up" data-aos-duration="1000">
          <h2 className='themeTitle text-2xl lg:text-4xl font-bold text-center'>{lang == "hindi" ? "संसाधन संग्रह":"Resource Library"}</h2>
        </div>
      </div>
      
<TabsComponent columns={columns} data={tableData} language={lang}/>
  </div>
</>
